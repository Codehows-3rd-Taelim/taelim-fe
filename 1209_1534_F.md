...
export default function AIChat() {
  ...
  const send = async (overrideText?: string) => {
    ...
    setMessages(prev => [...prev, { rawMessage: message, senderType: "USER" }]);
    setInput("");
    ...
    while (true) {
      ...
      // 새 대화면 conversationId 전달됨
      if (chunk.includes("conversationId") && !currentId) {
        tempId = chunk.replace("event: conversationId\ndata:", "").trim();
        setCurrentId(tempId);
      }

      setMessages(prev => [...prev, { rawMessage: chunk, senderType: "AI" }]);
